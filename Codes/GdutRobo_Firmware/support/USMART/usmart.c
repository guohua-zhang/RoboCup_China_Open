...
{
    __HAL_TIM_CLEAR_FLAG(&TIM7_Handler,TIM_FLAG_UPDATE);//清除中断标志位 
    __HAL_TIM_SET_AUTORELOAD(&TIM7_Handler,0XFFFF);     //将重装载值设置到最大
    __HAL_TIM_SET_COUNTER(&TIM7_Handler,0);             //清空定时器的CNT
...
{
	if(__HAL_TIM_GET_FLAG(&TIM7_Handler,TIM_FLAG_UPDATE)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=__HAL_TIM_GET_COUNTER(&TIM7_Handler);
...
{ 		    		  			       
    if(__HAL_TIM_GET_IT_SOURCE(&TIM7_Handler,TIM_IT_UPDATE)==SET)//溢出中断
    {
        ...
        __HAL_TIM_SET_COUNTER(&TIM7_Handler,0);;    //清空定时器的CNT
        __HAL_TIM_SET_AUTORELOAD(&TIM7_Handler,100);//恢复原来的设置
    }
...
{ 
    ...
    HAL_NVIC_SetPriority(TIM7_IRQn,3,0);    //设置中断优先级，抢占优先级3，子优先级0
    ...
    HAL_TIM_Base_Start_IT(&TIM7_Handler); //使能定时器4和定时器4中断 					 
}
